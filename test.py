...
def print_log(mae, rmse, mape, loss, state):
    state_str = "{} -------------------------------------".format(state)
    res_cycle = f"C-MAE: {mae[0]:.4f}, C-RMSE: {rmse[0]:.4f}, C-MAPE: {mape[0]:.2f}%"
    res_flow = f"F-MAE: {mae[3]:.4f}, F-RMSE: {rmse[3]:.4f}, F-AAE: {mae[4]:.4f}"
    print(state_str)
    print(res_cycle)
    print(res_flow)
    logger.info(state_str)
    logger.info(res_cycle)
    logger.info(res_flow)
...
st_time = time.time()
'''save model'''
PARAM_SAVE_PATH = './model_paras/{}/{}/'.format(args.cityname, args.model)
# paras_save = {
